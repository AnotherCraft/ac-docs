...
## Comments
WOGLAC uses standard C++ comment syntax. Block comments cant be nested however.
```WOGLAC
...
* Standard numeric literals.
  * Numeric literals can be optionally prefixed with either `~` or `#`.
...
| Syntax | Description |
| - | - |
|`cond ? then : else` | Ternary expression (`then` can be omitted, in that case `cond` doubles as `then`) |
| `||, &&` | Logical and/or |
| `==, !=, >, >=, <, <=` | Comparison |
| `+, -` | Add/subtract |
| `*, /` | Multiply/divide |
| `+a, -a, !a` | Unary +/- (+ does nothing, - negates), logical not |
| `(a)` | Parentheses |
...
# Standard procgen
Standard procgen is basically all about defining the `resultBlock` and `chunkZOffset` variables:
```WOGLAC
Float chunkZOffset = ...;
Block resultBlock = ...;
```
...
## Variables
> ```g4
> variableDefinition:	type=Type id=extendedIdentifier '=' val=expression ';';
> ```
...
* `PC` | Per chunk dimensionality
  * Fields with this dimensionality have the same value in each chunk (16 × 16 × entire Z axis, aligned).
...
1. The world is split into 2D array of tile size 256 × 256 blocks (16 × 16 chunks) (TODO: configurable, multiple biome channels)
...
* This expansion traversal is done in a depth-first manner, meaning the system first attempts to fully expand a current expansion candidate with all its dependencies before it gives up and tries another candidate.
